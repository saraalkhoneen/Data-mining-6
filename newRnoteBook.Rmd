...
chisq.test(tbl11)
```
Here we put the attribute 'class' values and the attribute 'employment' values in a table where the rows represent 'class' and the columns 'employment', then we checked the chi square for the table.
The p-value we got as a result of this test was 0.001045 which is less than 0.05. Based on it, we can say that the employment status influences credit risks which is logically correct.
...
chisq.test(tbl12)
```
Here we put the attribute 'class' values and the attribute 'savings_status' values in a table where the rows represent 'class' and the columns 'savings_status', then we checked the chi square for the table.
The p-value we got as a result of this test was  2.761e-07 which is much less than 0.05. which implies that the savings status influences credit risks strongly.
...
chisq.test(tbl13)
```
Here we put the attribute 'class' values and the attribute 'credit_history' values in a table where the rows represent 'class' and the columns 'credit_history', then we checked the chi square for the table.
The p-value we got as a result of this test was 1.279e-12 which is much less than 0.05. and based on it, credit history strongly affect the credit risks.
...
chisq.test(tbl14)
```
Here we put the attribute 'class' values and the attribute 'checking_status' values in a table where the rows represent 'class' and the columns 'checking_status', then we checked the chi square for the table.
The p-value we got as a result of this test was 2.2e-16 which is much less than 0.05. Based on this number we can say thet credit satatus strongly affect the credit risks.
...
On the other hand, fold 10 gave the highest (with Information Gain) and the lowest (with Ratio Gain) accuracy on the same time, we believe that fold ten data's characteristics was well fitted for the Information Gain split but it might triggered the sensitivity of the Ratio Gain tree.
