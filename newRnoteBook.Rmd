...
## clustring: 
Clustering is the process of organizing a collection of objects in a manner where items within the same group (cluster) share similarities, distinguishing them from objects in other groups (clusters). In the upcoming section, we aim to partition our dataset using the k-means algorithm, exploring three distinct k-means values (2, 3, and 4). For each trial, we will assess the average silhouette, total within-cluster sum of squares, and BCubed metrics (precision and recall).
