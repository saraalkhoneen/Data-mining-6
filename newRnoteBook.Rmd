...
```
From this bar chart we can see that we do have an balanced dataset, with 70% 'good' and 30% 'bad' labeled records this dataset is acceptable as it is and we do not need to take further actions regarding this matter.
...
We will construct 9 different decision tree using 3 different algorithms, Gini Index with Cart algorithm, Gain Ratio using C4.5 algorithm and Information Gain using ID3 algorithm. This section contains 3 different part for each tree, inside each part there is 3 different trees using 3 different folds (3,5,10) and the results we got from this tree construction.
...
for (i in 1:k) {
 ...
 gini <- 1 - sum((table(predictions) / length(predictions))^2)
  
  cat(sprintf("Fold: %d, Accuracy: %.2f%%, Gini Index: %.4f\n", i, accuracy * 100, gini))
}
```
in fold 8 The model achieved a relatively high accuracy of 77.78% indicating that it performed well in predicting the class labels for the test data in this fold.
The Gini Index for Fold 8 is 0.2975. This value suggests that the decision tree's splits in this fold are reasonably pure, contributing to the model's effectiveness in distinguishing between classes.
Compared to other folds, Fold 8 shows a good balance between accuracy and purity of splits. It has a higher accuracy than Folds 2 and 5, and a lower Gini Index than Fold 2. This suggests that the decision tree in Fold 8 achieved a good trade-off between predictive power and purity of the splits.
Fold 9 exhibits a higher accuracy than most other folds, indicating that the decision tree in this fold is particularly effective in making accurate predictions. The Gini Index is moderate, suggesting a balance between predictive power and the purity of splits.
...
```
The accuracy Indicates the proportion of correctly classified instances. in Fold 5 , 74.87% of the instances were classified correctly.
The Gini Index Measures the impurity or disorder in the dataset. Fold 3 has the highest Gini Index (0.3661), suggesting that the split created in this fold is less pure compared to others. while in fold 4 the gini index  (0.2121) suggesting that the split created in this fold is the purest compared to others.
...
```
In Fold 3, the accuracy is 74.32%, and the Gini Index decreased to 0.3082. A lower Gini Index is generally desirable, indicating a purer split. This suggests that the model performed relatively well on this subset, and the data might be more separable or less impure.
...
# Perform cross-validation with C5.0
for (i in 1:num_folds) {
 
# Define the percentage for training data
train_percentage <- 0.50 # 80% for training, 20% for testing

# Create an index for splitting
split_index <- sample(1:nrow(dataset), size = round(train_percentage * nrow(dataset)))
...
testing_set <- dataset[-split_index, ]
  ...
  tree_model <- C5.0(myFormula, data = training_set)
...
```{r}
plot(tree_model, uniform = TRUE, main = "C4.5 Tree - Fold 10")
...
for (i in 1:num_folds) {
  ...
  # Train the rpart model with information split method
  tree_model <- rpart(myFormula, data = training_set, method = 'class', parms = list(split = 'information'))

  # Make predictions on the testing set
  predictions <- predict(tree_model, newdata = testing_set, type = 'class')

  # Evaluate the model (you can customize this based on your evaluation criteria)
  confusion_matrix <- table(predictions, testing_set$class)
  accuracy <- sum(diag(confusion_matrix)) / sum(confusion_matrix)
...
## information gain usning ID3 algorithm 
...
# Assuming 'dataset' is the data frame

# Define the number of folds
num_folds <- 3

# Sets up the control parameters for cross-validation
train_control <- trainControl(method = "cv", number = num_folds)

# Builds an ID3 decision tree 
 id3_model <- rpart(class ~ checking_status + duration + credit_history + purpose + credit_amount + savings_status + employment + installment_commitment + personal_status + other_parties + residence_since + property_magnitude + age + other_payment_plans + housing + existing_credits + job + num_dependents + own_telephone + foreign_worker, data = dataset, method = "class", control = rpart.control(cp = 0.01), parms = list(split = "information"), xval = num_folds)

# printing the tree's info
print(id3_model)

#plotting the tree
 prp (id3_model, uniform = TRUE, main = "Pruned Decision Tree")
text(id3_model, use.n = TRUE, all = TRUE, cex = 0.6)

# Making Predictions 
new_data <- data.frame(dataset)

predictions <- predict(id3_model, newdata = new_data, type = "class")

# Confusion Matrix:
confusion_matrix <- table(predictions, new_data$class)
...
numeric_data <- dataset[, numeric_attributes]
categorical_data <- dataset[, categorical_attributes]

# Normalize numeric data (optional but often recommended)
...
# Create a list to store k-means plots
kmeans_plots <- list()
...
for (k in k_values) {
  ...
  # Plot the clusters for k-means with adjusted margins
  par(mar = c(5, 4, 4, 2) + 0.1)
  kmeans_plot <- ggplot(data = as.data.frame(normalized_numeric_data), aes(x = duration, y = credit_amount, color = factor(kmeans_model$cluster))) +
    geom_point(size = 3) +
    labs(title = paste("K-Means Clustering Results (k =", k, ")"), x = "Duration", y = "Credit Amount", color = "Cluster") +
    theme_minimal()
  ...
  kmeans_plots[[as.character(k)]] <- kmeans_plot
}

# Display k-means plots
kmeans_plots
...
# Plot the clusters for k-medoids with adjusted margins
par(mar = c(5, 4, 4, 2) + 0.1)
kmedoids_plot <- ggplot(data = as.data.frame(categorical_data), aes(x = factor(1), y = factor(2), color = factor(kmedoids_model$cluster))) +
  geom_point(size = 3) +
  labs(title = "K-Medoids Clustering Results", x = "Category 1", y = "Category 2", color = "Cluster") +
  theme_minimal()
